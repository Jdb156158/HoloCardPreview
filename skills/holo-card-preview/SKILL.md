...
Uploaded images stay in browser memory as object URLs. There is no upload API, persistence or cross-tab synchronization. Shared `export.js` exports front-face PNG, 4-second GIF and MP4/WebM video locally; retain bundled `vendor` libraries and licenses. Test a generated preview, cancellation and the phone export panel after export changes. The animation is a designed light orbit with slight affine motion, not a recording of user gestures or exact CSS 3D perspective. Keep the page visible during encoding. The default illustration is AI-generated; replace it only when requested. Publishing the template is separate from publishing private user artwork: use only assets authorized for that destination.
